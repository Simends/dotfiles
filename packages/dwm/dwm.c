...
deck(Monitor *m) {
	...
	if (m->pertag->drawwithgaps[m->pertag->curtag]) { /* draw with fullgaps logic */
	    ...
	    for(i = 0, my = m->pertag->gappx[m->pertag->curtag], c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		    if(i < m->nmaster) {
			    h = (m->wh - my) / (MIN(n, m->nmaster) - i) - m->pertag->gappx[m->pertag->curtag];
			    resize(c, m->wx + m->pertag->gappx[m->pertag->curtag], m->wy + my, mw - (2*c->bw) - m->pertag->gappx[m->pertag->curtag]*(5-ns)/2, h - (2*c->bw), False);
			    my += HEIGHT(c) + m->pertag->gappx[m->pertag->curtag];
		    }
		    else
			    resize(c, m->wx + mw + m->pertag->gappx[m->pertag->curtag]/ns, m->wy + m->pertag->gappx[m->pertag->curtag], m->ww - mw - (2*c->bw) - m->pertag->gappx[m->pertag->curtag]*(5-ns)/2, m->wh - (2*c->bw) - 2*m->pertag->gappx[m->pertag->curtag], False);
	    ...
	    for(i = my = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		    if(i < m->nmaster) {
			    ...
			    resize(c, m->wx + mw - c->bw, m->wy, m->ww - mw - c->bw, m->wh - c->bw, False);
    }
}
...
col(Monitor *m)
{
	...
	if (m->pertag->drawwithgaps[m->pertag->curtag]) { /* draw with fullgaps logic */
	    ...
	    for (i = 0, x = y = m->pertag->gappx[m->pertag->curtag], c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
	        if (i < m->nmaster) {
	            w = (mw - x) / (MIN(n, m->nmaster) - i) - m->pertag->gappx[m->pertag->curtag];
	            resize(c, x + m->wx, m->wy + m->pertag->gappx[m->pertag->curtag], w - (2*c->bw) + m->pertag->gappx[m->pertag->curtag], m->wh - (2*c->bw) - (2*m->pertag->gappx[m->pertag->curtag]), 0);
	            x += WIDTH(c) + m->pertag->gappx[m->pertag->curtag];
	        } else {
	            h = (m->wh - y) / (n - i) - m->pertag->gappx[m->pertag->curtag];
	            resize(c, x + m->wx, m->wy + y, m->ww - x - (2*c->bw) - m->pertag->gappx[m->pertag->curtag], h - (2*c->bw), 0);
	            y += HEIGHT(c) + m->pertag->gappx[m->pertag->curtag];
	        }
	} else { /* draw with singularborders logic */
	    if (n > m->nmaster)
		    mw = m->nmaster ? m->ww * m->mfact : 0;
...
tstack(Monitor *m) {
	...
	for (i = mx = 0, tx = m->wx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			...
			resize(c, m->wx + mx, m->wy + h, w - (2 * c->bw), mh - (2 * c->bw), 0);
			mx += WIDTH(c);
		} else {
